...
import { NudgeUIAnimation } from './NudgeUIAnimation';

// Define content for the clickable cards
const showcaseCardsData = [
  {
    id: 'line',
    title: 'AI-Powered Financial Nudges',
    description: 'Trigger customized push notifications to your users based on income and expense patterns with MoneyOne\'s TSP Analytics layer, driving proactive financial decisions and engagement.',
    duration: 8, // Example duration in seconds
  },
  {
    id: 'pie',
    title: 'Account & Investment Nudges',
    description: 'Automatically prompt users to start a new SIP based on average EOD balance, maintain funds for upcoming payments, switch to better-performing funds, or track periodic inflows like rent.',
    duration: 8, // Example duration in seconds
  },
  {
    id: 'bar',
    title: 'Insurance & Lifestyle Nudges',
    description: 'Detect spending at pharmacies to suggest health insurance, identify loan EMIs to propose loan cover or vehicle insurance, and track external investments in a held away portfolio.',
    duration: 8, // Example duration in seconds
  },
];
...
export const NudgesInsightsShowcase = () => {
  const [activeCardId, setActiveCardId] = useState<'line' | 'pie' | 'bar'>(showcaseCardsData[0].id as 'line' | 'pie' | 'bar');
  ...
  const handleCardClick = (cardId: string, direction = 1) => {
    ...
    setActiveCardId(cardId as 'line' | 'pie' | 'bar');
  ...
  return (
    ...
    >
      ...
      <div className={cn("relative flex justify-center items-center", isSmallScreen ? "w-full h-[650px] overflow-hidden" : "w-full md:w-2/3 min-h-[650px]")}>
        {/* The Phone Mockup is now persistent and outside of AnimatePresence */}
        <NudgeUIAnimation activeCardId={activeCardId} />
      </div>
    </motion.div>
  );
};

export default NudgesInsightsShowcase; 